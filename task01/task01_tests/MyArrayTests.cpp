...
BOOST_FIXTURE_TEST_SUITE(MyArray_, EmptyArray)
	...
	BOOST_AUTO_TEST_CASE(test_constructor_displacement)
	{
		CMyArray<int> arr1 = { 1,2,34 };
		std::vector<int> numbers = { 1, 2, 34 };
		CMyArray<int> arr2 = arr1;
		BOOST_CHECK(arr1.IsEmpty());
		VerifyArray(arr2, numbers);
	}
	...
	BOOST_AUTO_TEST_CASE(resize_array_with_start_item)
	{
		...
		size_t size1 = 10;
		arr.Resize(size1, 0);
		BOOST_CHECK_EQUAL(arr.GetSize(), size1);
		VerifyArray(arr, numbers);
		arr.Resize(2, 0);
